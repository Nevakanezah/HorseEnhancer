		...
		if(((father instanceof Horse && mother instanceof Donkey)
		  ...
		  || ((father instanceof Donkey && mother instanceof Horse)
			...
			maximule = true;
				...
				&& ((Horse)father).getInventory().getArmor().getType().equals(Material.GOLDEN_HORSE_ARMOR)
				&& mother.hasPotionEffect(PotionEffectType.INVISIBILITY)
				&& ((Horse)mother).getInventory().getArmor().getType().equals(Material.GOLDEN_HORSE_ARMOR)) {
			invincible = true;
		}
